...
<body>
                    ...
                    <div class="collapse navbar-collapse justify-content-between" id="navbarCollapse">
                        <div class="navbar-nav mr-auto py-0">
							...
							if($personname == ''){
								...
								<a href="about.php" class="nav-item nav-link">Acerca De</a>
								<a href="shop.php" class="nav-item nav-link">Cursos</a>
								<a href="course.php" class="nav-item nav-link active">Mis Cursos</a>
								<a href="contact.php" class="nav-item nav-link">Contacto</a>
								';
							}
							?>
    ...
    <div class="container-fluid pt-5 pb-3">
        <h2 class="section-title position-relative text-uppercase mx-xl-5 mb-4"><span class="bg-secondary pr-3">Mis Cursos</span></h2>
        <div class="row px-xl-5">
			<?php
			$contd = 0;
			$query_detail = "select * from detail where codeperson = '$codeperson' and detailstate = 'PAGADO' order by rand() limit 8";
			$result_detail=db_query($query_detail);
			while($row_detail = mysqli_fetch_array($result_detail))
			{ 	 	 	 	 	 	 	 	 	 	 	 	
				$contd++;	  	 	 	 	 	 	 		 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	
				$codedetail = $row_detail['codedetail'];
				$codecourse = $row_detail['codecourse'];
				$codeperson = $row_detail['codeperson'];
				$detailname = $row_detail['detailname'];
				$detailcourse = $row_detail['detailcourse'];
				$detailprice = $row_detail['detailprice'];
				$detailtype = $row_detail['detailtype'];
				$detailstate = $row_detail['detailstate'];
				
				$row_course = db_fetch_array (db_query("select coursename, courseprice, courseimg from course where codecourse = '$codecourse'"));
				list ($coursename, $courseprice, $courseimg) = $row_course;
				
				echo"
				<div class='col-lg-3 col-md-4 col-sm-6 pb-1'>
					<div class='product-item bg-light mb-4'>
						<div class='product-img position-relative overflow-hidden' onclick='ref_detailp($codecourse)' style='cursor: pointer;'>
							<img class='img-fluid w-100' src='admin/resource/images/course/$courseimg' alt=''>
							
						</div>
						<div class='text-center py-4'>
							<a class='h6 text-decoration-none text-truncate' href='detailp.php?codecourse=$codecourse'>".wordwrap($coursename, 25, "<br />\n")."</a>
    ...
    <div class="container-fluid pt-5 pb-3">
        <h2 class="section-title position-relative text-uppercase mx-xl-5 mb-4"><span class="bg-secondary pr-3">Cursos Recientes</span></h2>
        <div class="row px-xl-5">
			<?php
			$contc = 0;
